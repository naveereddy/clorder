...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    if (![[modifierDic objectForKey:@"clientFields"] isKindOfClass:[NSNull class]]) {
        //        NSLog(@"%lu",[[modifierDic objectForKey:@"clientFields"] count]);
        return [[modifierDic objectForKey:@"clientFields"] count];
    }else
        return 0;
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == choicesTbl) {
        ...
        if (indexPath.section == 0) {
            ...
            if (indexPath.row == 0){
                return [self heightForSelectedModifier]+25;
            }
            if (indexPath.row == 1){
                return 100;
            }
...
//            [cell.contentView addSubview:descLbl];
            ...
            if (indexPath.row == 0) {
                UILabel *instuctions = [[UILabel alloc] initWithFrame:CGRectMake(20, 25, SCREEN_WIDTH-60, [self heightForSelectedModifier])];
                instuctions.textColor = [UIColor lightGrayColor];
                instuctions.numberOfLines = 0;
                instuctions.font = [UIFont fontWithName:@"Lora-Regular" size:14];
                instuctions.text = [NSString stringWithFormat:@"%@", [self getSelectedModifier]];
                //                instuctions.delegate = self;
                
                titleLbl.textColor = [UIColor blackColor];
                titleLbl.text = [NSString stringWithFormat:@"%@", @"Choose"];
                
                UIButton *pickerBtn = [UIButton buttonWithType:UIButtonTypeCustom];
                [pickerBtn setImage:[UIImage imageNamed:@"choose-btn"] forState:UIControlStateNormal];
                pickerBtn.frame = CGRectMake(SCREEN_WIDTH-40, 10, 30, 30);
                [pickerBtn setBackgroundColor:DARK_GRAY];
                pickerBtn.layer.cornerRadius = 5.0;
                [pickerBtn addTarget:self action:@selector(pickerBtnAct) forControlEvents:UIControlEventTouchUpInside];
                
                [cell.contentView addSubview:titleLbl];
                [cell.contentView addSubview:instuctions];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [Moditbl deselectRowAtIndexPath:indexPath animated:NO];
}
